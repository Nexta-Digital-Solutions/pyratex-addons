odoo.define('ecommerce_product.add_to_cart', function (require) {
    ...
    const getSelectedProductIds = require('ecommerce_product.add_to_the_pack').getSelectedProductIds;
        ...
        async function getIdFromProductProduct(selectedProductIds) {
        let result;

        await rpc.query({
            model: 'product.product',
            method: 'search_read',
			args: [[ ['id','in', selectedProductIds]], ['id', 'list_price'] ]
        }).then(function (data) {
...
//}
    ...
    async function getProductId(productIds) {
        var results = [];
...
//    }
    ...
    $(document).on('click', '#o_add_to_Cart', async function () {
        ...
        try {
            ...
            const resultId = await getIdFromProductProduct(selectedProductIds);
